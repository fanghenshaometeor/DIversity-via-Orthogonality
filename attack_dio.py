    ...
    if args.attack_type == 'fgsm':
        ...
        for idx in range(args.num_heads):
            ...
            for eps in fgsm_epsilons:
                ...
                corr_te_fgsm, _, _ = attack(backbone, head, idx, testloader, attack_param)
        ...
        for idx in range(args.num_heads):
            ...
            for eps in pgd_epsilons:
                attack_param['eps'] = eps
                corr_te_pgd, _, _ = attack(backbone, head, idx, testloader, attack_param)
        ...
        for idx in range(args.num_heads):
            print("-------- attacking network-%d..."%idx)
            corr_te_cw, _, _ = attack(backbone, head, idx, testloader, attack_param)
        ...
        for idx in range(args.num_heads):
            print("-------- attacking network-%d..."%idx)
            corr_te_aa, _, _ = attack(backbone, head, idx, testloader, attack_param)
            break
...
# -------- PGD attack --------
def pgd_attack(backbone, head, idx, image, label, eps, alpha=0.01, iters=7, random_start=True, d_min=0, d_max=1):
...
def attack(backbone, head, idx, testloader, attack_param):
    ...
    for test in testloader:
        ...
        if args.attack_type == "fgsm":
            perturbed_image, _ = fgsm_attack(backbone, head, idx, image, label, attack_param['eps'])
        elif args.attack_type == "pgd":
            perturbed_image, _ = pgd_attack(backbone, head, idx, image, label, attack_param['eps'])
        elif args.attack_type == "cw":
