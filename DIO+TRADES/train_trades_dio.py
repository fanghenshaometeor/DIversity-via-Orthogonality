...
def train_epoch(backbone, head, trainloader, optim, criterion, epoch):
    ...
    for batch_idx, (b_data, b_label) in enumerate(trainloader):
        ...
        # ==============================
        # ======== TRADES + DIO ======== 
        loss_trades, x_adv = trades_dio_loss(backbone=backbone, head=head,
                           x_natural=b_data,
                           y=b_label,
                           optimizer=optim,
                           step_size=args.train_gamma,           # 2/255=0.007
                           epsilon=args.train_eps,               # 8/255=0.031
                           perturb_steps=args.train_step,        # 10
                           beta=args.beta_trades)        
        # ======== TRADES + DIO ========
        # ==============================
        # ==============================
        ...
        total_loss = loss_trades + args.alpha * loss_ortho + args.beta * loss_margin
        ...
        losses_trades.update(loss_trades.float().item(), b_data.size(0))
    ...
    print('     TRADES  loss = %f.'%losses_trades.avg)
