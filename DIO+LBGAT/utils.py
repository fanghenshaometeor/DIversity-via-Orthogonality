...
def get_model_teacher(args):
    ...
    if args.teacher_model == 'preactresnet18':
        net = preactresnet.__dict__[args.teacher_model](num_classes=num_class)

    elif args.teacher_model == 'resnet18':
        net = resnet.__dict__[args.teacher_model](num_classes=num_class)
    
    elif 'wrn' in args.teacher_model:
        net = wideresnet.__dict__[args.teacher_model](num_classes=num_class)
    else:
        assert False, "Unknown model : {}".format(args.arch)
...
def get_model_dio_teacher(args):
    if args.dataset == 'CIFAR10':
        num_class = 10
        dataset_normalization = NormalizeByChannelMeanStd(
            mean=[0.4914, 0.4822, 0.4465], std=[0.2470, 0.2435, 0.2616])
    ...
    if args.teacher_model == 'preactresnet18':
        backbone, head = dio_preactresnet.__dict__[args.teacher_model](num_classes=num_class,num_classifiers=args.num_heads)
    elif args.teacher_model == 'resnet18':
        backbone, head = dio_resnet.__dict__[args.teacher_model](num_classes=num_class,num_classifiers=args.num_heads)
    elif 'wrn' in args.teacher_model:
        backbone, head = dio_wideresnet.__dict__[args.teacher_model](num_classes=num_class)
    else:
        assert False, "Unknown model : {}".format(args.arch)
